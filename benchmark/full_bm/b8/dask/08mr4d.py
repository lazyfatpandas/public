...
def myfunc():
    ...
    # load files
    ratings_df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b2/s4.csv', low_memory=False)
    metas_df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b2/movies_metadata.csv', low_memory=False,dtype={'budget': 'object',
                                                                                                           'id': 'object',
                                                                                                           'popularity': 'object',
                                                                                                           'revenue': 'float64',
                                                                                                           'vote_count': 'float64'})
    links_df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b2/links.csv', low_memory=False,dtype={'tmdbId': 'Int64'})
    # 1st join
...
if __name__=='__main__':
    ...
    writer.writerow(["b8", "4.2GB", "s4.csv","dask",time2,format(ram_usage),845])
    file.close()
