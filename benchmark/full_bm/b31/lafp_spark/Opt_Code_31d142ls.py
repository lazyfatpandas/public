...
def myfunc():
    ...
    df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b31/s2.csv',usecols=SO_columns)
    print(df.shape)
    ...
    sns.countplot(y='department',data=df.compute(live_df=[df]).to_pandas())
    ...
    print(df.isnull().sum())
    sns.lmplot(x='satisfaction',y='last_evaluation',data=df[(df.status == 'Left')].compute(live_df=[df]).to_pandas(),fit_reg=False)
    ...
    # df = pd.get_dummies(df.compute(live_df=[df]),columns=['department','salary'])
    print(df.head(10))
    print(df)
    print((time() - start))
    pd.flush()
...
if (__name__ == '__main__'):
    ...
    writer.writerow(['b31','450MB','s2.csv','lspark',time2,format(ram_usage),3128])
    file.close()
