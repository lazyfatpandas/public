...
//Commented parts are marked with text Infolab, IIT Bombay, or venkateshek
    ...
    private void runPacksNormally() {
        ...
        if (Options.v().interactive_mode()){
            if (InteractionHandler.v().getInteractionListener() == null){
                G.v().out.println("Cannot getExpr in interactive mode. No listeners available. Continuing in regular mode.");
        ...
        for (SootClass s : appClasses) {
            ...
            for (SootMethod m : s.getMethods()) {
                ...
                if(m.hasActiveBody()){
                    ...
                    if(transformations){
//                        body.analyzeAST(); //commented by @venkatesh for wrong static-ness error ("this" erroneously gets treated as a variable in the resulting code. need to fix)
         ...
         */
        if(transformations){
//            InterProceduralAnalyses.applyInterProceduralAnalyses();//commented by @venkatesh for wrong static-ness error ("this" erroneously gets treated as a variable in the resulting code. need to fix)
        }
    ...
    private void outputDava(){
        ...
        /*
        Modified by IIT Bombay - Infolab
        We do not need the build file to be generated.
        (Soot throws CompilationDeath exception trying to generate build.xml file
        in some cases.)
        So we are commenting the below code
         */
        /*******                      COMMENTING STARTS                      ***********/
        /*
    ...
    private void runBodyPacks(SootClass c) {
        ...
        for (SootMethod m : methodsCopy) {
            ...
            if (produceJimple) {
                ...
                /* Modified by Infolab, IIT Bombay */
                /*******                      COMMENTING STARTS                      ***********/
