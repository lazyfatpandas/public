...
def myfunc():
    start = time()
    train_df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b20/s4_train.csv',dtype={"text_id":"string","full_text":"string", \
                                                                                   "cohesion":       "Float64", \
                                                                                   "syntax":         "Float64", \
                                                                                   "vocabulary":     "Float64", \
                                                                                   "phraseology":    "Float64", \
    ...
    test_df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b20/s4_test.csv')
    ...
    # ## Word Count
    #spark error
    train_df['word_count'] = train_df.full_text.apply(lambda x: len(x.split()) if not pd.isna(x) else 0)
    # Mean word count:
    # LAFP: print added
    print(train_df['word_count'].mean())
    # Max word count:
    print(train_df['word_count'].max())
    pd.set_option('compute.ops_on_diff_frames', False)
...
if __name__=='__main__':
    ...
    writer.writerow(["b20", "4.2GB", "s4.csv","spark",time2,format(ram_usage),2047])
    file.close()
