...
def myfunc():
    ...
    t = con.read_csv("/media/bhushan/nvme1/data/vldb/b16/s5.csv", separator=";")
    ...
    yes_result_p = t[t['loan_status'] == 'yes']['previous_loan_status']
    ...
    branch_1 = pl.DataFrame({
        'customer_id': ['1', '2', '3', '4', '5'],
        'first_name': ['Andrew', 'Alex', 'Sabestian', 'Hilary', 'Jack'],
        'last_name': ['Ng', 'Hales', 'Rachaska', 'Masan', 'Anthony']
    })
    branch_2 = pl.DataFrame({
        'customer_id': ['4', '5', '6', '7', '8'],
        'first_name': ['Brain', 'Steve', 'Kim', 'Steve', 'Ben'],
        'last_name': ['Alexander', 'Jobs', 'Jonas', 'Fleming', 'Richardsan']
    })
    credit_score = pl.DataFrame({
        'customer_id': ['1', '2', '3', '4', '5', '7', '8', '9', '10', '11'],
        'score': [513, 675, 165, 961, 1080, 1654, 415, 900, 610, 1116]
    })

    ib_branch_1 = ibis.memtable(branch_1)
    ib_branch_2 = ibis.memtable(branch_2)
    ib_credit_score = ibis.memtable(credit_score)
    print(ib_credit_score.execute())
    # 19. Concatenate branches
    df_new = ib_branch_1.union(ib_branch_2)
    print(df_new.execute())

    # 20. Merge new with credit score
    merged = df_new.join(ib_credit_score, ["customer_id"])
...
if __name__=='__main__':
    ...
    writer.writerow(["b16", "12.6GB", "s5.csv","polars",time2,format(ram_usage),"165B"])
    file.close()
