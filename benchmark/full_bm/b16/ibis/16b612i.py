...
def myfunc():
    ...
    t = con.read_csv("/media/bhushan/nvme1/data/vldb/b16/s2.csv", delim=";")
    ...
    value_normalize = value_count_res.mutate(**{col_name: value_count_res[col_name] / value_count_res[col_name].sum()}).order_by(ibis.desc(col_name))

    print(value_normalize.execute())
    
    print(
        t.filter(t.loan_status == True)
...
if __name__=='__main__':
    ...
    writer.writerow(["b16", "450MB", "s2.csv","ibis",time2,format(ram_usage),1629])
    file.close()
