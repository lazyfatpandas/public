...
def myfunc():
    start = time()
    import numpy as np
    df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b16/s2.csv',delimiter=';')
    print(df.head())
    print(df.describe())
    ...
    print(df['job'].unique().compute())
    # Counts for different types of `job`
    print(df['job'].value_counts().compute())
    ...
    # LAFP: print added
    print(df[df['loan_status'] == 'yes']['education'].value_counts(normalize=True).compute())
    # ## Check the percentage of loan approved by `previous loan status`
    print(df[df['loan_status'] == 'yes']['previous_loan_status'].value_counts(normalize=True).compute())
    ...
    print(marital_status_summery.compute())
    # ## Create a  Dataframes
    #
    # class 1
    branch_1 = {
    ...
    #Bhushan Modified: Dask error - not supported -- falling to pandas
    df_branch_1 = pdp.DataFrame(branch_1, columns = ['customer_id', 'first_name', 'last_name'])
    # df_branch_1 = pdp.DataFrame(branch_1, columns = ['customer_id', 'first_name', 'last_name'])
    print(df_branch_1)
    ...
    #Bhushan Modified: Dask error - not supported -- falling to pandas
    df_branch_2 = pdp.DataFrame(branch_2, columns = ['customer_id', 'first_name', 'last_name'])
    print(df_branch_2)
    ...
    #Bhushan Modified: Dask error - not supported -- falling to pandas
    df_credit_score = pdp.DataFrame(credit_score, columns = ['customer_id','score'])
    ...
    #
    print(pd.merge(df_new, df_credit_score, left_on='customer_id', right_on='customer_id').compute())
    print(time()-start)
...
if __name__=='__main__':
    ...
    writer.writerow(["b16", "450MB", "s2.csv","dask",time2,format(ram_usage),1625])
    file.close()
