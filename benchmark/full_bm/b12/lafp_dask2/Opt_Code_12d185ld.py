...
def myfunc():
    ...
    df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b12/s5.csv',usecols=SO_columns)
    print(df.head())
    ...
    df_grouped = df.groupby('Industry').size().sort_values(ascending=False)
    print(df_grouped)
...
if (__name__ == '__main__'):
   ...
   writer.writerow(['b12','12.6GB','s5.csv','ldask',time2,format(ram_usage),1256])
   file.close()
