...
def myfunc():
    start = time()
    start = time()
    df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s3_city.csv',dtype={'NUMBER': 'object',
                                                                            'UNIT': 'object'})
    # LAFP: print added
    print(df.head())
    ...
    print(df.compute())
    df2 = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s3_pluto.csv')
    # LAFP: print added
    print(df2.head(1))
...
if __name__=='__main__':
    ...
    writer.writerow(["b21", "1.4GB", "s3.csv","dask",time2,format(ram_usage),2135])
    file.close()
