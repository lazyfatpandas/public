...
def myfunc():
    start = time()
    start = time()
    df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s5_city.csv',dtype={'NUMBER': 'object',
                                                                            'UNIT': 'object'})
    # LAFP: print added
    print(df.head())
    ...
    # df.to_csv('manhattan_bk_addresses.csv')
    print(df.compute())
    df2 = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s5_pluto.csv')
    # LAFP: print added
    print(df2.head(1))
    #create a dictionary of addresses grouped by zipcode
    # LAFP: print added, to_dict() not supported by dask
    # print(df2.groupby('zipcode')['address'].apply(lambda q: q.values.tolist()).to_dict())
    print(df2.groupby('zipcode')['address'].apply(lambda q: q.values.tolist()).compute())
    # print(df2.compute())
...
if __name__=='__main__':
    ...
    writer.writerow(["b21", "12.6GB", "s5.csv","dask",time2,format(ram_usage),2155])
    file.close()
