...
def myfunc():
    ...
    df_ea = df.loc[(df['Industry'].isin(env) | df['Industry'].isin(ai))].reset_index(drop=True)
    df_ea['MainIndustry'] = df_ea.Industry.apply(lambda x: set_MainIndustry(x),meta=('MainIndustry', 'object'))
...
if (__name__ == '__main__'):
    ...
    writer.writerow(['b19','abl','s3.csv','ldask',time2,format(ram_usage),1936])
    file.close()
