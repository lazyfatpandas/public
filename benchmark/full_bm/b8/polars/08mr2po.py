...
def myfunc():
    ...
    # load files
    ratings = con.read_csv('/media/bhushan/nvme1/data/vldb/b2/s2.csv')
    ...
    step1 = links2.select("movieId", "imdbId_int").join(
        ratings.select("movieId", "rating"),
        predicates=[links2.movieId == ratings.movieId],
        how="right"
    ).select(
...
if __name__=='__main__':
    ...
    writer.writerow(["b8", "450MB", "s2.csv","polars",time2,format(ram_usage),"82B"])
    file.close()
