...
def myfunc():
  start = time()
  print = pd.lazyPrint
  SO_c_d_t = {"LON":"float32","LAT":"float32","NUMBER":"str","STREET":"category","UNIT":"category","CITY":"category","DISTRICT":"category","REGION":"category","POSTCODE":"float32","ID":"int64","HASH":"str"}
  df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s3_city.csv',dtype=SO_c_d_t)
  print(df.head())
  ...
  df2 = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s3_pluto.csv',dtype=SO_c_d_t)
  print(df2.head(1))
  pd.flush()
  print((time() - start))
...
if (__name__ == '__main__'):
   ...
   writer.writerow(['b21','1.4GB','s3.csv','lpolars',time2,format(ram_usage),'213C'])
   file.close()
pd.flush()
