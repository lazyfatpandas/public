...
def myfunc():
    ...
    # load files
    ratings = con.read_csv('/media/bhushan/nvme1/data/vldb/b2/s1.csv')
    ...
    step1 = links1.select("movieId", "imdbId_int").join(
        ratings.select("movieId", "rating"),
        predicates=[links1.movieId == ratings.movieId],
        how="right"
    ).select(
...
if __name__=='__main__':
    ...
    writer.writerow(["b8", "150MB", "s1.csv","polars",time2,format(ram_usage),"81B"])
    file.close()
