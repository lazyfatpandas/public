...
def myfunc():
    ...
    df_riders_data_to_load = pd.read_csv('/media/bhushan/nvme1/data/vldb/b33/s5_ride.csv',dtype=SO_c_d_t)
    ...
    plt.scatter(x_urban.compute(live_df=[df_city_data_to_load,merge_table_df,x_suburban,y_suburban,s_suburban,x_rural,y_rural,s_rural,s_urban,y_urban]).to_numpy(),y_urban.compute(live_df=[df_city_data_to_load,merge_table_df,x_suburban,y_suburban,s_suburban,x_rural,y_rural,s_rural,s_urban]).to_numpy(),label='Urban',s=(s_urban.compute(live_df=[df_city_data_to_load,merge_table_df,x_suburban,y_suburban,s_suburban,x_rural,y_rural,s_rural]).to_numpy() * 10),color=['pink'],edgecolor='black',alpha=1,marker='o')
    plt.scatter(x_rural.compute(live_df=[df_city_data_to_load,merge_table_df,x_suburban,y_suburban,s_suburban,s_rural,y_rural]).to_numpy(),y_rural.compute(live_df=[df_city_data_to_load,merge_table_df,x_suburban,y_suburban,s_suburban,s_rural]).to_numpy(),label='Rural',s=(s_rural.compute(live_df=[df_city_data_to_load,merge_table_df,x_suburban,y_suburban,s_suburban]).to_numpy() * 10),color=['purple'],edgecolor='black',alpha=1,marker='o')
    plt.scatter(x_suburban.compute(live_df=[df_city_data_to_load,merge_table_df,s_suburban,y_suburban]).to_numpy(),y_suburban.compute(live_df=[df_city_data_to_load,merge_table_df,s_suburban]).to_numpy(),label='Suburban',s=(s_suburban.compute(live_df=[df_city_data_to_load,merge_table_df]).to_numpy() * 10),color=['orange'],edgecolor='black',alpha=1,marker='o')
    plt.grid()
    ...
    plt.pie(fare_sum.compute(live_df=[df_city_data_to_load,type_grouped]).to_numpy(),explode=explode,labels=labels,colors=colors,autopct='%1.1f%%',shadow=True,startangle=150)
    ...
    explode = (0.1,0.1,0.1)
    colors = ['lightblue','lightgreen','lightcoral']
    plt.pie(rides_count.compute(live_df=[df_city_data_to_load]).to_numpy(),explode=explode,labels=labels,colors=colors,autopct='%1.1f%%',shadow=True,startangle=150)
    ...
    explode = (0.1,0.1,0.1)
    colors = ['lightblue','lightgreen','lightcoral']
    plt.pie(drivers_sum.compute(live_df=[]).to_numpy(),explode=explode,labels=labels,colors=colors,autopct='%1.1f%%',shadow=True,startangle=150)
...
if (__name__ == '__main__'):
    ...
    writer.writerow(['b33','12.6GB','s5.csv','lspark',time2,format(ram_usage),3358])
    file.close()
pd.flush()
