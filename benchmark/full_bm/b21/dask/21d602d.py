...
def myfunc():
    start = time()
    start = time()
    df = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s2_city.csv',dtype={'NUMBER': 'object',
                                                                            'UNIT': 'object'})
    # LAFP: print added
    print(df.head())
    ...
    print(df.compute())
    df2 = pd.read_csv('/media/bhushan/nvme1/data/vldb/b21/s2_pluto.csv')
    # LAFP: print added
    print(df2.head(1))
...
if __name__=='__main__':
    ...
    writer.writerow(["b21", "450MB", "s2.csv","dask",time2,format(ram_usage),2125])
    file.close()
